...
export const getColumnValueById = async (props: getColumnValueById) => {
  ...
  const { data, error } = await supabase.from("Events").select(columnName).eq("id", id);
...
export const getAllServerEvents = async (props: getAllServerEvents) => {
  ...
  const { data, error } = await supabase.from("Events").select("*").eq("serverId", getServerIdData![0]["id"]);
